...
public class UserController {
    ...
    public CommonResponse<Void> updatePassword(@RequestBody UpdatePasswordRequest request) {
        // 校验旧密码是否正确
        boolean isValid = accountService.validatePassword(request.getUsername(), request.getOldPassword());
        if (!isValid) {
            throw new RuntimeException("Old password is incorrect");
        }

        // 更新密码
        accountService.updatePassword(request.getUsername(), request.getNewPassword());
